...
function App() {
  ...
  async function enterParticipant() {
    try {
      setLoading(true)
      if (!supabase) throw new Error('Supabase não configurado. Confira as variáveis da Vercel.')
      ...
      <main className="shell">
        ...
        <div className="grid">
          <div className="card">
            ...
            <select value={official.result} onChange={(event) => setOfficial({ ...official, result: event.target.value })}>
            ...
            <label>Placar</label>
            <div className="score">
              <input type="number" min="0" value={official.brazil_score} onChange={(event) => setOfficial({ ...official, brazil_score: event.target.value })} />
              <span>x</span>
              <input type="number" min="0" value={official.japan_score} onChange={(event) => setOfficial({ ...official, japan_score: event.target.value })} />
            </div>

            <label>Jogador do primeiro gol</label>
            <select value={official.first_goal_player} onChange={(event) => setOfficial({ ...official, first_goal_player: event.target.value })}>
              {PLAYERS.map((player) => (
          ...
          <div className="card">
            ...
            <img className="qr" src={`https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=${encodeURIComponent(window.location.href)}`} alt="QR Code" />
    ...
    <main className="shell">
      ...
      <div className="grid">
        ...
        <div className="card">
          ...
          <label>Resultado</label>
          <select value={betForm.result_pick} onChange={(event) => setBetForm({ ...betForm, result_pick: event.target.value })}>
            <option>Brasil</option>
            <option>Empate</option>
            <option>Japão</option>
          </select>
          ...
          <input type="number" min="0" max="10" step="0.5" value={betForm.result_points} onChange={(event) => setAllocation('result_points', event.target.value)} />

          <label>Placar exato</label>
          <div className="score">
            <input type="number" min="0" value={betForm.brazil_score} onChange={(event) => setBetForm({ ...betForm, brazil_score: event.target.value })} />
            <span>x</span>
            <input type="number" min="0" value={betForm.japan_score} onChange={(event) => setBetForm({ ...betForm, japan_score: event.target.value })} />
          </div>

          <label>Pontos no placar</label>
          <input type="number" min="0" max="10" step="0.5" value={betForm.score_points} onChange={(event) => setAllocation('score_points', event.target.value)} />

          <label>Jogador do primeiro gol</label>
          <select value={betForm.first_goal_player} onChange={(event) => setBetForm({ ...betForm, first_goal_player: event.target.value })}>
            {PLAYERS.map((player) => (
              <option key={player}>{player}</option>
            ))}
          </select>
          ...
          <input type="number" min="0" max="10" step="0.5" value={betForm.player_points} onChange={(event) => setAllocation('player_points', event.target.value)} />
